...
from app.services.ollama_service import qa_with_ollama_using_search
...
@router.post("/qa/search", response_model=str)
def qa_with_search(query: str = Query(..., description="Search query to find relevant documents."),
                   question: str = Query(..., description="Question to ask about the retrieved context.")):
    """
    ...
    """
    try:
        return qa_with_ollama_using_search(query, question)
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))

@router.post("/add-document")
async def add_document(doc_id: str, content: str, relationships: list):
    try:
        graph_service.add_document(doc_id, content, relationships)
...
async def generate_response(query: str):
    try:
        response = graph_service.generate_response(query)
